...
| Field | Type | Default | Description |
|---|---|---|---|
| `source` | string | **required** | Where to read the secret value. One of `env` (environment variable) or `aws_sm` (AWS Secrets Manager). |
| `config` | object | **required** | Source-specific configuration. See [`env` config](#env-source) and [`aws_sm` config](#aws_sm-source) below. |
...
| `require` | boolean | `false` | When `true`, requests matching a configured rule are rejected with HTTP 403 if the proxy token is not present (replace mode) or if the target header/query param is already set by the client (inject mode). Prevents workloads from bypassing secret management. |
...
Read the secret from AWS Secrets Manager. The value is cached and refreshed lazily based on the configured TTL.
...
| `ttl` | duration | `5m` | How long to cache the secret value before refreshing from Secrets Manager. |
