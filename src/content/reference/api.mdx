...
Secrets let you store credentials for supported providers. The egress proxy injects real secrets into outbound requests on behalf of workloads, so the workload never sees the actual credential. Instead, workloads use a `proxy_value` placeholder in environment variables, and the proxy swaps it for the real secret at the network level.
...
| `provider`| string | yes      | The service this secret is for. One of: `github`, `npm`.                                                                                     |
| `secret`  | string | yes      | The secret value. Encrypted at rest. Never returned in responses.                                                                            |
| `env_var` | string | yes      | Environment variable name to set inside sandboxes (e.g. `GITHUB_TOKEN`). Must be uppercase letters, digits, and underscores, starting with a letter. Must be unique within the user's secrets. |
...
```json
{
  ...
  "provider": "github",
  "env_var": "GITHUB_TOKEN",
  "proxy_value": "IRONSH_PROXY_github_github-main",
  "comment": null,
...
The `proxy_value` field is the placeholder that gets set as the value of the environment variable inside sandboxes. The egress proxy replaces it with the real secret on outbound requests to the provider's hosts. The `proxy_value` is safe to expose — it is meaningless outside the proxy.

Creating a secret with a duplicate `name` returns `409 Conflict` with code `secret_name_taken`. Creating a secret with a duplicate `env_var` returns `409 Conflict`. Creating a secret with an unsupported provider returns `422` with code `validation_error`.
...
| `cursor`   | string  | —            | Pagination cursor from previous response       |
| `limit`    | integer | `50`         | Results per page. Max 200.                     |
| `sort`     | string  | `created_at` | Sort field: `created_at`, `name`, `provider`   |
| `order`    | string  | `desc`       | Sort order: `asc`, `desc`                      |
...
All fields are optional. Only provided fields are updated. `name` and `provider` cannot be changed after creation — delete and recreate instead.
