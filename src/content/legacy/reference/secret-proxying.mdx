...
- Every secret replacement is recorded in the [egress audit log](/legacy/reference/api#list-egress-audit-events), so you have a full trail of when and where credentials were injected.
...
2. Create a sandbox. The VM starts with `GITHUB_TOKEN=IRONSH_PROXY_github-main` in its environment.
