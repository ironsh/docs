...
If your iron-proxy config includes secrets (e.g. API keys for upstream services), you can inject them as environment variables using [AWS Secrets Manager](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/specifying-sensitive-data-secrets.html). Add a `secrets` block to the iron-proxy container definition:
...
ECS pulls the secret value at task launch and exposes it as an environment variable in the container. Don't forget to add a corresponding `secrets` entry in your `iron-proxy.yaml` so the proxy knows to read it. See the [configuration reference](/reference/configuration#secrets) for details.
