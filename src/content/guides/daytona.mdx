...
2. Configures iptables rules to redirect outbound HTTP and HTTPS traffic through iron-proxy.
...
Because iptables redirects all non-root outbound TCP traffic on ports 80 and 443, processes inside the sandbox cannot bypass the proxy regardless of how they make network requests. The container runs as the unprivileged `daytona` user, and sudo access is revoked after setup completes.
