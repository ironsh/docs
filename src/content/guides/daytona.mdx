...
2. Configures iptables rules to redirect outbound HTTP and HTTPS traffic through iron-proxy, and drops all other non-loopback traffic from non-root processes.
3. Points DNS at iron-proxy so all domain resolution flows through the proxy.
...
Because iptables redirects all non-root outbound TCP traffic on ports 80 and 443, and drops all other non-loopback traffic from non-root processes, processes inside the sandbox cannot bypass the proxy regardless of how they make network requests. The container runs as the unprivileged `daytona` user, and sudo access is revoked after setup completes.
...
# Redirect non-root TCP egress to iron-proxy
sudo iptables -t nat -A OUTPUT -m owner --uid-owner 0 -j RETURN
