...
In practice, this covers most workloads. Few applications connect to raw IP addresses. However, if you need to enforce strict egress control against adversarial code, you should use a platform that provides kernel-level egress control, or use [Freestyle's built-in network permissions](#freestyles-built-in-egress-control).
