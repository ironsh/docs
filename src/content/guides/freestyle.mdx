...
Freestyle VMs do not include the iptables `owner` match module (`xt_owner`). Without it, there is no straightforward way to distinguish traffic from iron-proxy vs. traffic from other processes, so you cannot add firewall rules that block direct IP connections without also blocking the proxy itself. If a process inside the VM connects to an IP address directly (bypassing DNS), iron-proxy cannot intercept that traffic. Egress control is limited to DNS-based interception only.
</Callout>
...
<Callout type="warning">
Freestyle VMs do not include the iptables `owner` match module (`xt_owner`), so you cannot write firewall rules that distinguish iron-proxy's traffic from other processes.
</Callout>

On platforms like [Amazon ECS](/guides/ecs) and [Daytona](/guides/daytona), you can use iptables with the `--uid-owner` match to allow traffic from iron-proxy (running as root) while blocking all other outbound traffic. This prevents workloads from bypassing the proxy by connecting to an IP address directly.

Freestyle's kernel does not include the `xt_owner` module, so this approach is not possible. Without a way to exempt iron-proxy's own traffic, any rule that blocks outbound connections would also block the proxy from reaching upstream servers. As a result, iron-proxy's egress control on Freestyle relies entirely on DNS interception: it works because `/etc/resolv.conf` points at iron-proxy, which resolves domains to its own address and terminates TLS. If a process hardcodes an IP address or uses its own DNS resolver, that traffic will bypass the proxy.
