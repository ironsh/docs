...
async function main() {
  const baseImage = new VmBaseImage("FROM debian:trixie-slim").runCommands(
    "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y curl ca-certificates openssl jq iptables"
  );
...
const result = await vm.exec("su - workload -c 'curl -s https://httpbin.org/get'");
...
```javascript
// Run commands as the unprivileged workload user
const result = await vm.exec("su - workload -c 'your-command-here'");
```
