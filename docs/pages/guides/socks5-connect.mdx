...
The tunnel listener MITM's TLS connections using the same CA as the standard HTTPS listener. Workloads connecting through the tunnel still need to trust iron-proxy's CA certificate. See [Managing CA Certificates](/guides/ca-certificates) for setup instructions.
