...
3. **Credential resolution.** `access_key_id`, `secret_access_key`, and `session_token` each resolve through their own [secret source](/credential-proxying/static-secrets#secret-sources).
...
6. **Header injection.** The proxy attaches the new `Authorization`, `X-Amz-Date`, and `X-Amz-Content-Sha256` headers. If a session token is configured, it also attaches `X-Amz-Security-Token`.
...
If the workload uses temporary credentials (assumed roles, STS), configure `session_token` as well:
...
| `session_token` | secret source | | Real session token for STS or assumed-role credentials. Omit for long-lived IAM users. |
...
- `injected`: the headers that were set. Always includes `header:Authorization`, `header:X-Amz-Date`, `header:X-Amz-Content-Sha256`, and `header:X-Amz-Security-Token` when a session token is configured.
