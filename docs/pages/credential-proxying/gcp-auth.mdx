...
1. **Keyfile resolution.** The service-account JSON loads either from disk (`keyfile_path`) or from any [secret source](/credential-proxying/overview#secret-sources) (`keyfile`). Set exactly one.
...
- **Service-account keyfiles only.** `gcp_auth` uses `JWTConfigFromJSON`, which only accepts service-account JSON. Workload identity federation and `external_account` credential configurations aren't supported.
