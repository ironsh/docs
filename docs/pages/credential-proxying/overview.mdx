...
| An AWS SigV4 signature | [AWS Request Signing](/credential-proxying/aws-auth) | An AWS access key, secret access key, and optional session token. |
| A Google service-account access token | [GCP Service Accounts](/credential-proxying/gcp-auth) | The service-account JSON keyfile. |
...
Credential proxying transforms run top-to-bottom in the order they appear under `transforms:`. Two rules matter most:
