...
Place the `judge` transform before the `secrets` transform so the LLM provider sees proxy tokens rather than real credentials. See [pipeline ordering](/policies/llm-judge#pipeline-ordering) for details.
...
See the [MCP Interception reference](/policies/mcp-interception) for a full overview, including matchers, audit log behavior, and current limitations.
