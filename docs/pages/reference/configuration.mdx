...
| Field | Type | Default | Description |
|---|---|---|---|
| `keyfile_path` | string | | Path to a Google service account JSON keyfile on disk. Mutually exclusive with `keyfile`. |
| `keyfile` | object | | Secret source that resolves to the keyfile JSON. Uses the same shape as a [secret source](#secret-sources). Mutually exclusive with `keyfile_path`. |
| `subject` | string | | Workspace user email to impersonate via domain-wide delegation. When set, the minted token acts as the subject rather than the service account. |
...
Exactly one of `keyfile_path` or `keyfile` must be set.
...
| Field | Type | Default | Description |
|---|---|---|---|
| `access_key_id` | object | **required** | Secret source resolving to the real AWS access key ID. |
| `secret_access_key` | object | **required** | Secret source resolving to the real AWS secret access key. |
