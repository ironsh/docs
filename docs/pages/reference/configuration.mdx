...
| Field | Type | Default | Description |
|---|---|---|---|
| `source` | object | **required** | Where to read the secret value. Contains a `type` field (`env`, `aws_sm`, `aws_ssm`, or `1password`) plus type-specific fields. See [sources](#secret-sources) below. |
...
Read the secret from 1Password using a service account token. Like the AWS sources, values are cached and refreshed in the background based on the TTL.
...
The `proxy_value`, `match_headers`, `match_body`, `match_path`, and `require` fields may also be set at the top level of a secret entry for backwards compatibility. New configurations should use the `replace` block.
