...
`tools/list` responses are filtered per-event so denied tools are removed from the agent's catalog. Filtering applies to both `application/json` responses and `text/event-stream` streamed responses. The agent only ever discovers tools that the policy allows, which keeps tool selection consistent with the enforced policy.
...
- **`judge`**: place an LLM judge before `secrets` if you want it to evaluate proxy tokens, as documented in the [LLM Judge reference](/reference/llm-judge#pipeline-ordering). The MCP interceptor runs after both.
