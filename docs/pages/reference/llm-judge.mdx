...
A single configuration can define many judge transforms. Each entry is an independent instance: its own `name`, `prompt`, `rules`, provider, semaphore, and circuit breaker. A failing or slow judge does not affect the others.
