...
:::warning
Keep `ca.key` out of source control. Any holder of this key can mint certificates that workloads will trust. See the [CA certificate reference](/guides/ca-certificates) for rotation guidance.
:::
...
If you control the workload image, you can also bake the CA into the system trust store at image build time. See the [CA certificate reference](/guides/ca-certificates) for per-runtime details.
