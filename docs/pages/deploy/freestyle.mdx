---
title: "Deploy on Freestyle"
description: "Run iron-proxy inside Freestyle VMs using pre-baked snapshots for fast, repeatable egress control."
---

# Deploy on Freestyle

Run iron-proxy inside [Freestyle](https://freestyle.sh) VMs. You create a VM snapshot with iron-proxy pre-installed, then use that snapshot to launch ephemeral VMs with egress control already configured.
...
The snapshot script trusts the CA system-wide and sets common runtime environment variables. Most tools will work without additional configuration. If you run into TLS errors, see the [CA certificate reference](/guides/ca-certificates) for per-runtime details.
